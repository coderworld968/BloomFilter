 ...
 */
@Beta
public final class CWBloomFilter<T> implements Predicate<T>, Serializable {
    /**
     ...
     * @since 12.0
     */
    public CWBloomFilter<T> copy() {
        return new CWBloomFilter<T>(bits.copy(), numHashFunctions, funnel, strategy);
    }
     ...
     * @since 15.0
     */
    public boolean isCompatible(CWBloomFilter<T> that) {
        checkNotNull(that);
        return (this != that)
     ...
     * @since 15.0
     */
    public void putAll(CWBloomFilter<T> that) {
        checkNotNull(that);
    ...
    public boolean equals(@NullableDecl Object object) {
        ...
        if (object instanceof CWBloomFilter) {
            CWBloomFilter<?> that = (CWBloomFilter<?>) object;
     ...
     * com.google.common.hash.BloomFilter} with false positive probability 3%.
     *
     ...
     * @since 23.0
     */
    public static <T> Collector<T, ?, CWBloomFilter<T>> toBloomFilter(
    ...
    /**
     * Returns a {@code Collector} expecting the specified number of insertions, and yielding a {@link
     * com.google.common.hash.BloomFilter} with the specified expected false positive probability.
     *
     * <p>Note that if the {@code Collector} receives significantly more elements than specified, the
     * resulting {@code BloomFilter} will suffer a sharp deterioration of its false positive
     * probability.
    ...
    /**
     * Creates a {@link com.google.common.hash.BloomFilter} with the expected number of insertions and expected false
     ...
     */
    public static <T> CWBloomFilter<T> create(
    ...
    public static <T> CWBloomFilter<T> create(
            Funnel<? super T> funnel, long expectedInsertions, double fpp) {
        return create(funnel, expectedInsertions, fpp, CWBloomFilterStrategies.MURMUR128_MITZ_64);
    }

    @VisibleForTesting
    static <T> CWBloomFilter<T> create(
        ...
        try {
            return new CWBloomFilter<T>(new LockFreeBitArray(numBits), numHashFunctions, funnel, strategy);
    ...
    /**
     * Creates a {@link com.google.common.hash.BloomFilter} with the expected number of insertions and a default expected
     ...
     * @return a {@code BloomFilter}
     */
    public static <T> CWBloomFilter<T> create(Funnel<? super T> funnel, int expectedInsertions) {
     ...
     * @since 19.0
     */
    public static <T> CWBloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions) {
    ...
    private static class SerialForm<T> implements Serializable {
        ...
        Object readResolve() {
            return new CWBloomFilter<T>(new LockFreeBitArray(data), numHashFunctions, funnel, strategy);
        }
     ...
     */
    public static <T> CWBloomFilter<T> readFrom(InputStream in, Funnel<? super T> funnel)
            throws IOException {
        ...
        try {
            ...
            Strategy strategy = CWBloomFilterStrategies.values()[strategyOrdinal];
